...
class VoiceAssistant:
    ...
    def setup_ai(self):
        ...
        self.model = genai.GenerativeModel(
            "gemini-2.0-flash",
            system_instruction="""You are purple, a chatbot designed for a college mini project assigned by Professor Reshma. You are being developed by Mueen and Nagesha, who are MCA students at BMSIT, Bangalore.
...
4. Stay in Character - You are purple, not an AI. Maintain""",
    ...
    def update_status(self):
        if self.awake:
            self.status_indicator.config(text="Status: Awake - Listening for command", foreground="#2ecc71")
            self.update_wave_animation()
        else:
            self.status_indicator.config(text="Status: Waiting for wake word", foreground=CONFIG['colors']['text_light'])
    ...
    def handle_command(self, command):
        try:
            ...
            if 'quit' in command or 'exit' in command:
                ...
                self.graceful_exit()
                return
            # Additional system control commands
            if 'open' in command:
                app_name = command.split('open ')[-1]
                # Check for special command "voice typing"
                if "voice typing" in command:
    ...
    def enable_voice_typing(self):
        try:
            # Simulate the Windows voice typing hotkey (Win+H)
            pyautogui.hotkey('win', 'h')
            self.speak("Voice typing enabled")
            self.add_to_chat("System", "Enabled voice typing", "ai")
        except Exception as e:
            self.speak("Failed to enable voice typing")
            self.show_error_ui(f"Error enabling voice typing: {e}")
